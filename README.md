# Same-Label Partner Averaging Can Inflate Unpaired Multi-Omics Fusion Estimates

Reproducible code and results for:

> **Same-Label Partner Averaging Can Inflate Unpaired Multi-Omics Fusion Estimates: A Pairing-Arithmetic Audit Applied to Alzheimer's Disease**
...
2. `bayes_error_corrected_bound_table11.py`: an observed-accuracy sensitivity version of the same bound, used only to support the one-sentence robustness claim in Section 5.4 ("led to the same qualitative conclusion throughout"); not tabulated in the appendix
3. `k_sweep.py`: empirical partner-count sweep, *k* = 1-50, against the sqrt(k) prediction (Table 9)
4. `k_sweep_uncertainty.py`: 95% seed-percentile intervals for the same sweep (Table 9's "seed range" column) and a k=20 seed-mean cross-check against the headline fixed-seed figure (Table 8)
5. `k20_seed_sensitivity.py`: canonical *k* = 20 partner-draw seed-sensitivity check (Section 5.4); sweeps 30 seeds of the standard convention's same-label fusion, quantifying how much the headline ceiling figure moves under an equally arbitrary alternative seed. Fully deterministic, verified bit-for-bit identical across repeated runs.
6. `seed_aware_inference.py`: the seed-averaged *k* = 1 bootstrap CI reported in the manuscript's main text (Section 5.5)
7. `k1_learned_model_all_thresholds.py`: canonical source for Table 10 and Table A5, the *k* = 1 single-partner construction and its MDE/TOST equivalence testing, at every confidence-margin threshold, using the same learned two-feature meta-model as `k_sweep.py` and `seed_aware_inference.py`. (An earlier version of this analysis used simple unweighted logit averaging instead of the learned meta-model, inconsistent with every other value of *k* in this pipeline; this script is the corrected, consistent version and should be used for anything *k* = 1 related.)
8. `regenerate_appendix_seed_arrays.py`: the per-seed accuracy and DeLong *p*-value distributions behind Appendix Figure A1, using the same corrected learned-meta-model method as (7)
9. `multiple_comparison_correction_v2.py`: Holm-Bonferroni and Benjamini-Hochberg correction across the full family of significance tests (Table A4)
10. `calibration_metrics.py` / `calibration_metrics_ci.py`: calibration slope, intercept, ECE, ICI and their bootstrap CIs across all five cohort/stage combinations (Table 6, Table A1)
11. `leave_one_series_out.py`: true leave-one-series-out retrain (Section 7, Limitations)
12. `score_fsqn_vs_baseline.py`: FSQN/ComBat vs. self-standardization scoring (Section 6.1). Pass `--external-combat <path>` for a three-way baseline/FSQN/ComBat comparison in one run
13. `sqrtk_simulation_and_controls.py`: three additions used for review-response validation — a controlled simulation of the sqrt(k) identity across standalone AUCs 0.55-0.90 (referenced in the abstract), an empirical variance-vs-*k* check on the real Stage-2 pool (Table A7, Fig. 3), and the same-label / opposite-label / random / label-permuted pairing controls (Section 5.3's prose)
14. `recalibrated_bound.py`: cross-fitted (nested) recalibration of Stage-1 posteriors and the resulting attainability bound under Platt and isotonic recalibration, across all four confidence-margin subsets (Table A6)
...
Each writes to `results/figures/`. The five main-text figures and one appendix figure read directly from `results/` CSVs and JSONs produced by the steps above.
